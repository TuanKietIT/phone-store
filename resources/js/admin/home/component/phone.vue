...
<script setup>
  ...
  const createPhone = () => {
    ...
    axios.post('/api/phone/create', formData,{
        headers: {
...
<template>
  <div class="flex min-h-screen w-full bg-gray-100 font-sans">
        ...
        <div class="w-2/5 bg-white ">
            <div class="flex flex-col pt-5">
                ...
                <div class="flex justify-between px-10 pt-2 ">
                    ...
                    <button type="submit" @click="createPhone" class="text-white bg-green-500 hover:bg-green-300 focus:outline-none font-medium text-sm rounded-lg px-5 py-2 text-center ml-5">Thêm</button>
                </div>
        ...
        <div class="flex w-3/5 flex-col gap-y-8">
          <div class="flex flex-col justify-between rounded-10 bg-white p-7">
            <div class="flex items-center justify-between">
              <h2 class="text-[20px] font-medium bg-white">All phone</h2>
            </div>
            <table class="mt-4">
              <thead>
                <tr>
              ...
              <tbody>
                <tr v-for="phone in phones.data" :key="phone.id" class="border-b bg-white last:border-none" >
                  <td class="py-4">
                  ...
                  <td class="py-4">
                    <span class="text-sm text-gray-400">
                      {{ moment(phone.created_at).format("YYYY-MM-DD") }}
                    </span>
                  </td>
                  <td class="py-4">
                  ...
                  <td class="py-4">
                    <div class="flex items-center gap-x-2">
                      <div @click.prevent="editPhone (phone)" class="flex items-center justify-center rounded-full bg-white p-2 hover:text-green-400 focus:text-green-400" >
                          <EditIcon lass="h-6 w-6 stroke-current text-gray-400 cursor-pointer"/>
                      </div>
                      <div @click.prevent="deletePhone (phone)" class="flex items-center justify-center rounded-full bg-white p-2 hover:text-red-400 focus:text-red-400" >
            ...
            <div class="flex items-center justify-center mt-10">
              <pagination class="" :data="phones" v-bind:showDisabled="true" icon="chevron" v-on:change-page="getResults"></pagination>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-show="showUpdate" class="fixed inset-0 bg-black bg-opacity-25 backdrop-blue-sm flex justify-center items-center">
        <div class="w-[500px]"> 
            <div class="bg-white">
                <Form @submit="updatePhone " :validation-schema="schema" v-slot="{errors}" class="flex flex-col space-y-5 " >
                    ...
                    <div class="flex justify-center items-center">
                        ...
                        <button @click="closePhone " class="text-white my-5 bg-green-500 hover:bg-green-300 focus:outline-none font-medium text-sm rounded-lg px-5 py-2.5 text-center mr-5">Cancel</button>
                    </div>
              </Form>
            </div>
        </div>
